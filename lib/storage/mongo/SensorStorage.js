...
export default class MongoSensorStorage extends SensorStorage
{
    ...
    async findAndUpdateValue(time, rawValue)
    {
        const value = this.normalizeValue(rawValue);
        const doc = await this.getDocumentOfTime(time);
        const minutes = time.getMinutes();

        const set = {};
        set[`values.${minutes}`] = value;
        const stats = doc.stats;
        this.updateStats(stats, rawValue);
        set["stats"] = stats;
    ...
    async getDocumentOfTime(time)
    {
        ...
        if (!doc)
        {
            doc = await this.insertBlankDocumentOfTimestamp(timestamp);
            return doc;
        }
        else
        {
    ...
    async insertBlankDocumentOfTimestamp(timestamp)
    {
        let doc = {
            timestamp,
            values: this.createBlankValues(),
            stats: this.createBlankStats()
        };
    ...
    createBlankValues()
    {
        if (this.sensor.config.monitor.interval === 60 * 1000)
        {
            ...
            {
                const value = this.createBlankValue();
                values.push(value);
            }
            return values;
        }
